...
timeDiodOn = EEPROM_ulong_read(1);
  if (timeDiodOn >= 24) {
    timeDiodOn = 0;
    EEPROM_ulong_write(1, delayLight);
  }




delayLight = EEPROM_ulong_read(0);
  if (delayLight > 300000) {
    delayLight = 10000;
    EEPROM_ulong_write(0, delayLight);
  }
  displays.clear();
  if (! rtc.begin()) {
...
void loop() {
  ...
  if (button.update()) { //если произошло событие
    if (button.read() == 1) { //если кнопка нажата
      ...
      if ((millis() - pressed_moment) < pressed_long) {  // если кнопка была нажата кратковременно
        ...
        displays.println(utf8rus(" постоянно"));
        displays.setFont(Adafruit5x7);
        AlwaysOnTime = millis(); 
        while (lightsOnW) {
        ...
        displays.clear();
        digitalWrite(LED, LOW);  //выключить светодиод
        digitalWrite(LAMP, HIGH); //выключить лампу
      ...
      { // кнопка удерживалась долго, начинаем установку времени и даты
        ...
        while (setStartDiodOn) {         //цикл пока не установили время скоторого начинает работать диод и нажали длинно кнопку
          ...
          displays.setFont(System5x7);
          displays.print(utf8rus("Час ДиодВкл"));
          displays.setFont(Adafruit5x7);
          displays.print(".");
          if (button.update())   { //если произошло событие
            if (button.read() == 1)
            ...
            { // кнопка отпущена
              ...
              { // кнопка удерживалась долго
                ...
                pressed_moment = 0;
                EEPROM_ulong_write(1, timeDiodOn);
                break;
              }
            }
          }
  ...
  byte datchdvig1 = digitalRead(PIR1); //digitalRead(PIR1);//смотрим не включен ли датчик движения1
  byte datchdvig0 = digitalRead(PIR);//смотрим не включен ли датчик движения

  DateTime now = rtc.now();  //получаем время и дату

  if (now.hour() >= timeDiodOn and now.hour() <= timeDiodOff) { //Если ночь и сработал датчик движения и светодиод не включен
    if (datchdvig0 == HIGH or datchdvig1 == HIGH) {
      if (lights == false) {
  ...
  if (now.hour() >= timeDiodOn and now.hour() <= timeDiodOff) {
    if ( datchdvig0 == LOW and datchdvig1 == LOW ) {
      if (lights == true) {
        if (millis() - lightOnTime >= delayLight) { //если ночь и датчик движения никого не видит и светодиод включен 30сек или больше
          digitalWrite(LED, LOW);  //выключить светодиод
          if (digitalRead(LAMP) == LOW) digitalWrite(LAMP, HIGH);         //выключить лампу при переходе за 0 часов
  ...
  if (now.hour() > timeDiodOff) { //если день
    if (datchdvig0 == HIGH or datchdvig1 == HIGH) {//и cработал датчик движения
      if (lights == false) {//и свет не включен
        digitalWrite(LAMP, LOW); //включить лампу
  ...
  if (now.hour() > timeDiodOff) {         //если день
    if (datchdvig0 == LOW and datchdvig1 == LOW) { //и никого не видно
      if (lights == true) { //и лампа включена
